<template>
  <div class="page-container">
    ...
    <div class="cards">
      <div class="card">
        <div class="card-label">统计用户数</div>
        <div class="card-value">{{ totals.userCount }}</div>
      </div>
      <div class="card">
      ...
      <div class="card">
        <div class="card-label">已付款单数</div>
        <div class="card-value">{{ totals.paidCount }}</div>
      </div>
      <div class="card">
    ...
    <el-table :data="rows" border stripe v-loading="loading" class="table">
      ...
      <el-table-column prop="totalHours" label="总时长(h)" width="110" align="right" sortable />
      <el-table-column prop="totalAmount" label="总金额(元)" width="120" align="right" sortable />
      <el-table-column prop="paidCount" label="已付款单" width="110" align="right" sortable />
      <el-table-column prop="paidAmount" label="已付款金额(元)" width="140" align="right" sortable />
      <el-table-column prop="canceledCount" label="已取消" width="90" align="right" sortable />
    </el-table>
  </div>
</template>
...
const totals = computed(() => {
  const userCount = rows.value.length
  const sum = (key) =>
