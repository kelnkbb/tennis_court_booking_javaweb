...
<template>
  <div class="home-container">
    <div class="welcome-card">
      <h1 v-if="isAdmin">欢迎使用网球场地管理系统</h1>
      <h1 v-else>欢迎来到网球场地预约系统</h1>
      <p v-if="isAdmin">选择左侧菜单开始管理</p>
      <p v-else>您可以查看场地并管理自己的预约</p>
    </div>
...
const loadStats = async () => {
  try {
    ...
    // 并行请求基础数据
    const basePromises = [getAllCourts()]
    ...
    if (isAdmin.value) {
      ...
      const myBookingsResult = results[1]
